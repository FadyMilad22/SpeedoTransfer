                    ...
                    BottomNavigationItem(
                        ...
                        onClick = {
                            ...
                            navController.navigate(
                                when (screen.name) {
                                    "Home" -> {
                                        "${Route.HOME}/${account.id.toLong()}/${account.createdAt}}/${getCurrentDate()}/${account.balance.toFloat()}/${customer.name}/${account.currency}"
                                    }

                                    "Transfer" -> {
                                        "${Route.BEGIN_TRANSACTION}/${customer.name}/${account.accountNumber}/${account.currency}/${tokenType} ${authToken}"
                                    }

                                    "Transactions" -> {
                                        "${Route.TRANSACTIONS_LIST}/${account.id.toLong()}/${account.createdAt}/${getCurrentDate()}"
                                    }

                                    "My card" -> {
                                        "$ACCOUNT_INFO/${account.accountDescription}/${account.accountName}/${account.accountNumber}/${account.accountType}/${account.active}/${account.balance}/${account.currency}"
                                    }

                                    else -> {
                                        "$MORE/${customer.id.toLong()}/${customer.name}/${customer.email}/${customer.birthDate}/Egypt/${tokenType} ${authToken}/${account.createdAt}"
                                    }
                                }
                            ) {
